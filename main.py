...
def filter_and_sort_entries(entries: List[dict]) -> List[dict]:
    ...
    if len(entries) > 1:
        ...
        priority_entries = [e for e in entries if not is_low_priority_version(e.get('title', ''))]
...
def download_single(
    ...
    try:
        ...
        progress.console.print("[bold red]Error:[/] yt-dlp is not installed. Run 'pip install yt-dlp'.")
    ...
    progress.console.print(f"[bold cyan]Processing:[/] [yellow]({line_num})[/] {entry_type} [white]{entry}[/]")

    try:
        with YoutubeDL(opts) as ydl:
            # First, extract information without downloading
            info_dict = ydl.extract_info(build_query(entry, top, is_specific_song), download=False)
            ...
            if info_dict and '_type' in info_dict and info_dict['_type'] == 'playlist':
                ...
                for item in entries:
                    ...
                    if title in existing_titles:
                        progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] Skipping: [dim]{item.get('title', 'Unknown')}[/] (already exists)")
                        continue
                ...
                if filtered_entries:
                    ...
                    items_preview = ", ".join([f"'{e.get('title', 'Unknown')[:30]}...'" for e in filtered_entries[:count]])
                    if len(filtered_entries) > count:
                        items_preview += f" and {len(filtered_entries) - count} more item(s)"
                    
                    progress.console.print(f"[blue]ℹ[/] [yellow]({line_num})[/] Selected [bold cyan]{len(filtered_entries)}[/] of [cyan]{len(entries)}[/] items from playlist: {items_preview}")
                    ...
                    for item in filtered_entries:
                        try:
                            ...
                            progress.console.print(f"[green]✓[/] [yellow]({line_num})[/] Downloaded: [bold]{title}[/]")
                            add_download_to_tracker(line_num, title)
                        except Exception as item_exc:
                            progress.console.print(f"[red]✗[/] [yellow]({line_num})[/] Failed to download an item from playlist: {str(item_exc)}")
                            # Remove from tracker if failed to try again later?
                            # No, as it could be a temporary failure. Keep it marked.
                    
                    progress.console.print(f"[green]✓[/] [yellow]({line_num})[/] Completed playlist: [bold]{playlist_title}[/] ([bold cyan]{len(filtered_entries)}[/] items)")
                else:
                    progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] No new items to download in playlist: [bold]{playlist_title}[/]")
                    ...
                    for item in filtered_entries:
                        ...
                        if should_download:
                            try:
                                ...
                                progress.console.print(f"[red]✗[/] [yellow]({line_num})[/] Failed to download: {str(exc)}")
                        else:
                            # Show appropriate message
                            if is_specific_song:
                                progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] Skipping: [dim]{item.get('title', 'Unknown')}[/] (already downloaded one song for this entry)")
                            else:
                                progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] Skipping: [dim]{item.get('title', 'Unknown')}[/] (artist already processed in this run)")
                    
                    if downloads_count == 0:
                        progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] No new songs downloaded for: [bold]{entry}[/]")
                else:
                    progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] No results for: [bold]{entry}[/] (or already downloaded)")
            else:
                progress.console.print(f"[yellow]⚠[/] [yellow]({line_num})[/] Could not find results for: [bold]{entry}[/]")
    except Exception as exc:
        progress.console.print(f"[bold red]✗[/] [yellow]({line_num})[/] Failed to process [yellow]'{entry}'[/]: {exc}")
    finally:
...
def main():
    ...
    parser.add_argument("-i", "--input", default="input.txt", help="Input file (default: input.txt)")
    parser.add_argument("-o", "--output", default="output", help="Destination directory (can be a flash drive)")
    parser.add_argument("-t", "--threads", type=int, default=4, help="Simultaneous downloads (default: 4)")
    parser.add_argument("--top", type=int, default=1, help="Number of results to download when the input is an artist (default: 1)")
    parser.add_argument("--force", action="store_true", help="Force download even of files that already exist")
    ...
    if not input_path.exists():
        console.print(f"[bold red]Error:[/] Input file '{input_path}' not found.", style="red")
        sys.exit(1)
    ...
    if not entries:
        console.print("[bold red]Error:[/] No valid entries found in input.txt.", style="red")
        sys.exit(1)
    ...
    if existing_titles and not args.force:
        console.print(f"[blue]ℹ[/] Found [bold cyan]{len(existing_titles)}[/] already downloaded files")
    ...
    for line_num, entry, is_specific_song in sorted(entries, key=lambda x: x[0]):
        ...
        if num_downloads > 3:
            display_list = ", ".join([f"'{item[:40]}...'" if len(item) > 40 else f"'{item}'" for item in downloaded[:3]])
